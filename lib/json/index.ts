...
export function jsonPretty(jsonString: string): JsonResult {
  try {
...
export function jsonMinify(jsonString: string): JsonResult {
  try {
    const parsed = JSON.parse(jsonString)
...
export function jsonFormat(jsonString: string, indent: number = 2): JsonResult {
  try {
    const parsed = JSON.parse(jsonString)
...
export function jsonPrettyString(jsonString: string): JsonResult {
  try {
    const parsed = JSON.parse(jsonString)
    let result = ""
...
export function jsonDiff(json1: string, json2: string): JsonResult {
  try {
    ...
    if (changes.length === 0) {
      return { result: "No differences found", error: null }
    }
