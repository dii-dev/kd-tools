...
import { soapToJson } from '@/lib/xml-utils'
import { AlertCircle, CheckCircle } from 'lucide-react'

export default function SoapToJsonPage() {
  const { t } = useLanguage()
      ...
      <div className="container mx-auto px-4 py-8 sm:py-10 md:py-12">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-8 sm:mb-10 md:mb-12">
            <h1 className="mb-3 text-3xl font-bold text-foreground sm:text-4xl md:text-5xl">{t('soap.json.title')}</h1>
            <p className="text-base text-muted-foreground sm:text-lg">{t('soap.json.subtitle')}</p>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-2 md:gap-8">
            <div>
              <JsonEditor
                value={input}
                onChange={setInput}
                label={t('soap.json.input')}
                placeholder={t('soap.json.placeholder-input')}
              />
            </div>

            <div>
              <JsonHighlightedView
                label={t('soap.json.output')}
                output={output}
                placeholder={t('soap.json.placeholder-output')}
                downloadFileName="soap-to-json-output.json"
              />
            </div>
          </div>
          ...
          {output && !error && (
            <div className="mt-6 flex gap-3 rounded-lg border border-green-200 bg-green-50 p-4 dark:border-green-800 dark:bg-green-950">
              ...
              <p className="text-sm text-green-700 dark:text-green-300">{t('soap.json.success')}</p>
            </div>
          )}
        </div>
      </div>
